...
et al. 2006). For example, individuals may be invited to screening and
given treatment according to the results of an imperfect test. Then
their long term outcomes are estimated, with the population split into
true positive, true negative, false positive and false negatives who may
or may not have agreed to take the treatment which may not have been
effective. Outcomes for the second stage may include progression to
disease, side effects from treatment, and mortality.
...
\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{swimline-new.png}
...
purposes of teaching and training in R (Lin et al. 2023). The example
will demonstrate the different implementation approaches in a real-world
case study. This is a CEA model that involves multiple screening
sessions over a lifetime where, at the point of every screening session,
individuals can be redistributed across health states in a MM based on
their screening results (Afonso et al. 2023; Hazen 2002; Hollenberg
1984).
...
screening model from Lin et al. (2023). The Markov model and decision
tree diagrams are shown in Figure~\ref{fig-markov-model} and
Figure~\ref{fig-decision-tree-model}.
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{model-diagram.png}

}

\caption{\label{fig-screening-model}Screening model diagram taken from
Lin et al. (2023). The white health states represent the natural history
of disease. The two interventions, screening and treatment, are labelled
in green. The screening intervention can only be implemented before the
individual enters clinical health state (stage 3), and treatment can be
received when patients are diagnosed from screening (stage 2) or
clinically presented (stage 3).}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{markov_model.png}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{decision_tree_model2.png}
...
\NormalTok{DecisionTree }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(data, }\AttributeTok{disc\_rate =} \FloatTok{0.035}\NormalTok{) \{}
  \FunctionTok{structure}\NormalTok{(}
    \FunctionTok{list}\NormalTok{(}\AttributeTok{data =}\NormalTok{ data,}
         ...
         \AttributeTok{start\_cycle   =} \DecValTok{0}\NormalTok{),}
...
\NormalTok{                        costs, utils, }
  ...
  \FunctionTok{structure}\NormalTok{(}
    \FunctionTok{list}\NormalTok{(}
      ...
      \AttributeTok{discount\_rate =}\NormalTok{ disc\_rate,}
      \AttributeTok{start\_cycle   =} \DecValTok{0}
\NormalTok{    ),}
...
\NormalTok{  out }\OtherTok{\textless{}{-}} \FunctionTok{markov\_model}\NormalTok{(}
    ...
    \AttributeTok{start\_cycle   =}\NormalTok{ model}\SpecialCharTok{$}\NormalTok{start\_cycle}
\NormalTok{  )}
...
\NormalTok{  path }\OtherTok{\textless{}{-}} \FunctionTok{calculate\_pathways}\NormalTok{(model}\SpecialCharTok{$}\NormalTok{data)}
  
\NormalTok{  rate }\OtherTok{\textless{}{-}}\NormalTok{ model}\SpecialCharTok{$}\NormalTok{discount\_rate}
\NormalTok{  cycle }\OtherTok{\textless{}{-}}\NormalTok{ model}\SpecialCharTok{$}\NormalTok{start\_cycle}
  
\NormalTok{  discount\_factor }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{/}\NormalTok{ (}\DecValTok{1} \SpecialCharTok{+}\NormalTok{ rate)}\SpecialCharTok{\^{}}\NormalTok{cycle}
  
  \FunctionTok{list}\NormalTok{(}
    \AttributeTok{expected\_cost =} \FunctionTok{sum}\NormalTok{(path}\SpecialCharTok{$}\NormalTok{prob }\SpecialCharTok{*}\NormalTok{ path}\SpecialCharTok{$}\NormalTok{cost) }\SpecialCharTok{*}\NormalTok{ discount\_factor,}
    \AttributeTok{expected\_eff  =} \FunctionTok{sum}\NormalTok{(path}\SpecialCharTok{$}\NormalTok{prob }\SpecialCharTok{*}\NormalTok{ path}\SpecialCharTok{$}\NormalTok{eff) }\SpecialCharTok{*}\NormalTok{ discount\_factor,}
    \AttributeTok{path\_results  =}\NormalTok{ path,}
...
\NormalTok{    total\_time }\OtherTok{\textless{}{-}}\NormalTok{ n\_scr }\SpecialCharTok{*}\NormalTok{ t\_val}
    
    \CommentTok{\# Filter out impossible combinations that}
    \CommentTok{\# exceed the 20{-}year time horizon}
    \ControlFlowTok{if}\NormalTok{ (total\_time }\SpecialCharTok{\textgreater{}}\NormalTok{ time\_horizon) }\ControlFlowTok{next}
    
\NormalTok{    submodels }\OtherTok{\textless{}{-}} \FunctionTok{rep}\NormalTok{(}\FunctionTok{list}\NormalTok{(dt, mm\_scenario), n\_scr)}
\NormalTok{    n\_models }\OtherTok{\textless{}{-}} \FunctionTok{length}\NormalTok{(submodels)}
    
\NormalTok{    t\_remain }\OtherTok{\textless{}{-}}\NormalTok{ time\_horizon }\SpecialCharTok{{-}}\NormalTok{ total\_time}
    \ControlFlowTok{if}\NormalTok{ (t\_remain }\SpecialCharTok{\textgreater{}} \DecValTok{0}\NormalTok{) \{}
\NormalTok{      submodels[[n\_models]]}\SpecialCharTok{$}\NormalTok{n\_cycles }\OtherTok{\textless{}{-}}\NormalTok{ t\_val }\SpecialCharTok{+}\NormalTok{ t\_remain}
\NormalTok{    \}}
...
implemented in packages such as \emph{vctrs} (Wickham et al. 2026).
Taking into account these considerations, in our case study example we
have placed the linking logic within the model running function. We have
also assumed that given a target model, we then know what the source
model is. This is reasonable in our context and keeps the code clean for
demonstration purposes. An alternative version of our code using the
double dispatch approach employed by \emph{vctrs} is also available in
the supplementary material. The newer and less adopted S7 OOP system in
R does natively support multiple dispatch which could elegantly solve
the problem with a little more advanced code. We provide a version of
...
\pandocbounded{\includegraphics[keepaspectratio]{manuscript_files/figure-pdf/fig-occupy-combined-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{manuscript_files/figure-pdf/fig-occupy-combined-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{manuscript_files/figure-pdf/fig-ce-plot-1.pdf}}
...
\section{Changes in Model and Data}\label{changes-in-model-and-data}

\subsubsection{Submodel substitution}\label{submodel-substitution}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# constructor}
\NormalTok{PartSAModel }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(surv\_pfs, surv\_os, cost\_vector, }
\NormalTok{                        qaly\_vector, }\AttributeTok{mapping =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{time\_horizon =} \DecValTok{10}\NormalTok{, ...) \{}
  \FunctionTok{structure}\NormalTok{(}
    \FunctionTok{list}\NormalTok{(}
      \AttributeTok{surv\_pfs     =}\NormalTok{ surv\_pfs,     }\CommentTok{\# parametric survival curves}
      \AttributeTok{surv\_os      =}\NormalTok{ surv\_os,}
      \AttributeTok{cost\_vector  =}\NormalTok{ cost\_vector,}
      \AttributeTok{qaly\_vector  =}\NormalTok{ qaly\_vector,}
      \AttributeTok{mapping      =}\NormalTok{ mapping,      }\CommentTok{\# links tree terminal nodes to baseline survival risk}
      \AttributeTok{time\_horizon =}\NormalTok{ time\_horizon}
\NormalTok{    ),}
    \AttributeTok{class =} \FunctionTok{c}\NormalTok{(}\StringTok{"PartSAModel"}\NormalTok{, }\StringTok{"Model"}\NormalTok{)}
\NormalTok{  )}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#\textquotesingle{} S3 update method: Ingests Decision Tree outputs into PartSA baseline parameters}
\CommentTok{\#\textquotesingle{} @export}
\NormalTok{update\_model.PartSAModel }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(model, result) \{}
  \CommentTok{\# Map tree probabilities to adjust scale or }
  \CommentTok{\# baseline hazard for each treatment arm}
\NormalTok{  model}\SpecialCharTok{$}\NormalTok{baseline\_weights }\OtherTok{\textless{}{-}} \FunctionTok{map\_decision\_to\_partsa}\NormalTok{(result, model}\SpecialCharTok{$}\NormalTok{mapping)}
\NormalTok{  model}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#\textquotesingle{} S3 runner method: Solves the area under the curves}
\CommentTok{\#\textquotesingle{} @export}
\NormalTok{run\_model.PartSAModel }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(model) \{}
  \CommentTok{\# Evaluate partitioned survival areas (PFS, OS {-} PFS, 1 {-} OS)}
\NormalTok{  out }\OtherTok{\textless{}{-}} \FunctionTok{run\_partsa\_engine}\NormalTok{(}
    \AttributeTok{pfs\_fit    =}\NormalTok{ model}\SpecialCharTok{$}\NormalTok{surv\_pfs, }
    \AttributeTok{os\_fit     =}\NormalTok{ model}\SpecialCharTok{$}\NormalTok{surv\_os, }
    \AttributeTok{weights    =}\NormalTok{ model}\SpecialCharTok{$}\NormalTok{baseline\_weights,}
    \AttributeTok{horizon    =}\NormalTok{ model}\SpecialCharTok{$}\NormalTok{time\_horizon}
\NormalTok{  )}
  
  \FunctionTok{structure}\NormalTok{(}
    \FunctionTok{list}\NormalTok{(}
      \AttributeTok{expected\_cost =} \FunctionTok{sum}\NormalTok{(out}\SpecialCharTok{$}\NormalTok{state\_costs }\SpecialCharTok{*}\NormalTok{ model}\SpecialCharTok{$}\NormalTok{cost\_vector),}
      \AttributeTok{expected\_eff  =} \FunctionTok{sum}\NormalTok{(out}\SpecialCharTok{$}\NormalTok{state\_qalys }\SpecialCharTok{*}\NormalTok{ model}\SpecialCharTok{$}\NormalTok{qaly\_vector),}
      \AttributeTok{survival\_mesh =}\NormalTok{ out}\SpecialCharTok{$}\NormalTok{mesh}
\NormalTok{    ),}
    \AttributeTok{class =} \FunctionTok{c}\NormalTok{(}\StringTok{"output"}\NormalTok{, }\FunctionTok{class}\NormalTok{(model))}
\NormalTok{  )}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Swap Markov for PartSA without touching runner code:}
\NormalTok{model\_c }\OtherTok{\textless{}{-}} \FunctionTok{PartSAModel}\NormalTok{(}\AttributeTok{surv\_pfs =}\NormalTok{ fit\_pfs, }\AttributeTok{surv\_os =}\NormalTok{ fit\_os, }
                       \AttributeTok{cost\_vector =}\NormalTok{ cost\_vec, }\AttributeTok{qaly\_vector =}\NormalTok{ util\_vec, }
                       \AttributeTok{mapping =}\NormalTok{ map\_ps)}
\NormalTok{chain\_partsa }\OtherTok{\textless{}{-}} \FunctionTok{CombinedModel}\NormalTok{(model\_a, model\_c)}

\CommentTok{\# Identical orchestration call}
\NormalTok{res\_partsa }\OtherTok{\textless{}{-}} \FunctionTok{run\_model}\NormalTok{(chain\_partsa)}
\end{Highlighting}
\end{Shaded}

\subsubsection{Structural Adaptations via S3 Class
Specialisation}\label{structural-adaptations-via-s3-class-specialisation}

When a jurisdiction mandates a structural change---such as an
alternative long-term extrapolation method or an additional
post-progression therapy branch---you use class derivation (e.g.,
\texttt{c("MarkovModel\_PBAC",\ "MarkovModel",\ "Model")}).

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# PBAC structural variant: models continuous PBS dynamic market{-}share erosion}
\NormalTok{MarkovModel\_PBAC }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(trans\_matrix, costs, utils, }
                             \AttributeTok{erosion\_rate =} \FloatTok{0.05}\NormalTok{, }\AttributeTok{mapping =} \ConstantTok{NULL}\NormalTok{, ...) \{}
\NormalTok{  mod }\OtherTok{\textless{}{-}} \FunctionTok{MarkovModel}\NormalTok{(}\AttributeTok{trans\_matrix =}\NormalTok{ trans\_matrix, }\AttributeTok{costs =}\NormalTok{ costs, }
                     \AttributeTok{utils =}\NormalTok{ utils, }\AttributeTok{mapping =}\NormalTok{ mapping, ...)}
\NormalTok{  mod}\SpecialCharTok{$}\NormalTok{erosion\_rate }\OtherTok{\textless{}{-}}\NormalTok{ erosion\_rate}
  \FunctionTok{class}\NormalTok{(mod) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"MarkovModel\_PBAC"}\NormalTok{, }\FunctionTok{class}\NormalTok{(mod))}
\NormalTok{  mod}
\NormalTok{\}}

\CommentTok{\# Specific S3 method dispatch: PBAC applies market{-}share price cuts over cycles}
\NormalTok{run\_model.MarkovModel\_PBAC }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(model) \{}
  \CommentTok{\# Apply country{-}specific dynamic erosion adjustment before running}
\NormalTok{  model}\SpecialCharTok{$}\NormalTok{cost\_vector }\OtherTok{\textless{}{-}} \FunctionTok{apply\_pbs\_erosion}\NormalTok{(model}\SpecialCharTok{$}\NormalTok{cost\_vector, model}\SpecialCharTok{$}\NormalTok{erosion\_rate)}
  
  \CommentTok{\# Delegate core execution to base Markov runner}
  \FunctionTok{NextMethod}\NormalTok{(}\StringTok{"run\_model"}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

\texttt{NextMethod()} delegates the heavy matrix operations directly to
the verified global engine (\texttt{run\_model.MarkovModel}). The
central orchestrator \texttt{run\_model.CombinedModel} runs without
touching a line of code.

\section{Discussion}\label{discussion}

Traditional health economic models often bundle clinical pathways, cost
inputs, and final calculations into a single, rigid script or
spreadsheet, making them fragile and difficult to update. In contrast,
our framework applies established software engineering
standards---specifically the SOLID principles---to create a much more
robust and adaptable system. By ensuring that each component has only
one job (Single Responsibility), such as completely separating the
clinical simulation from the economic payoffs, we drastically reduce the
risk of errors when updating parameters. Furthermore, the architecture
is designed so that new components, like a custom jurisdiction profile
or a new sub-model, can be plugged into the pipeline without altering or
breaking the core mathematical engine (Open-Closed Principle). Because
all sub-models communicate through a standardised data contract,
researchers can swap a Markov model for a different structure with
confidence that the downstream calculations will still function
perfectly (Liskov Substitution). Ultimately, this approach moves health
economic modelling away from bespoke, tangled code and towards a
transparent, modular infrastructure where each piece can be
independently tested and trusted.
...
Alarid-Escudero, Fernando, Eline M. Krijkamp, Eva A. Enns, et al. 2019.
{``A Framework for Decision-Analytic Modeling in {R}: Implementing
Tertiary Prevention of Colorectal Cancer with a {Markov} Model.''}
\emph{Medical Decision Making} 39 (2): 175--84.
...
Modelling for Health Economic Evaluation}. Oxford University Press.
...
Green, Nathan, Felicity Lamrock, Niamh Naylor, et al. 2023. {``Health
Economic Evaluation Using Markov Models in r for Microsoft Excel Users:
A Tutorial.''} \emph{PharmacoEconomics} 41 (1): 5--19.
...
Complex Markov-Processes.''} \emph{Medical Decision Making} 4: 529--29.

\bibitem[\citeproctext]{ref-husereau_consolidated_2022}
Husereau, Don, Michael Drummond, Federico Augustovski, et al. 2022.
{``Consolidated {Health} {Economic} {Evaluation} {Reporting} {Standards}
2022 ({CHEERS} 2022) {Statement}: {Updated} {Reporting} {Guidance} for
{Health} {Economic} {Evaluations}.''} \emph{Value in Health} 25
(January): 3--9. \url{https://doi.org/10.1016/j.jval.2021.11.1349}.
...
Kent, Seamus, Frauke Becker, Talitha Feenstra, et al. 2019. {``The
Challenge of Transparency and Validation in Health Economic Decision
Modelling: A View from Mount Hood.''} \emph{PharmacoEconomics} 37 (11):
1305--12. \url{https://doi.org/10.1007/s40273-019-00825-1}.
...
Sandercock, Peter, Eivind Berge, Martin Dennis, et al. 2004.
{``Cost-Effectiveness of Thrombolysis with Recombinant Tissue-Type
Plasminogen Activator for Acute Ischemic Stroke: Assessed by a Model
Based on {UK} {NHS} Costs.''} \emph{Stroke} 35 (6): 1490--97.
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{UML.png}
