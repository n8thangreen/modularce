...
run_partsa_engine <- function(pfs_fit, os_fit, weights, horizon) {
  # Calculate simple area under the curve (AUC) for time in states
  t <- seq(0, horizon, by = 1)
  auc_pfs <- sum(pfs_fit(t))
  auc_os  <- sum(os_fit(t))
  
  time_pf   <- auc_pfs
  time_pd   <- auc_os - auc_pfs
  time_dead <- horizon - auc_os
  
  # Map PartSA states to the 6 Markov states
  # PFS -> Healthy (disease free)
  # PD -> Diseases
  # 1 - OS -> Death_from_Disease & Other_Cause_Death
  state_time <- c(
    Healthy              = time_pf,
  ...
  list(
    ...
    mesh = cbind(PFS = pfs_fit(t), OS = os_fit(t))
  )
}
...
update_model.PartSAModel <- function(model, result) {
  model$baseline_weights <- rep(1, length(model$mapping))
  model
}
```
...
```{r PartSA-analysis}
# Swap Markov for PartSA
psm <- PartSAModel(surv_pfs = fit_pfs, surv_os = fit_os, 
                   cost_vector = cost_vec, qaly_vector = util_vec, 
                   mapping = map_ps)

screening_model <- CombinedModel(dt, psm)

# Identical orchestration call
res_screening_model <- run_model(screening_model)
```
