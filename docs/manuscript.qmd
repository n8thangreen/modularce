...
run_model.MarkovModel <- function(model, ...) {
  out <- markov_model(
    ...
    start_cycle   = ifelse(is.null(model$start_cycle), 0, model$start_cycle)
  )
...
run_model.DecisionTree <- function(model, ...) {
  ...
  start_cycle <- ifelse(is.null(model$start_cycle), 0, model$start_cycle)
  discount_factor <- 1 / (1 + model$discount_rate)^start_cycle
  
  list(
    expected_cost = sum(pathways$prob * pathways$cost) * discount_factor,
    expected_eff  = sum(pathways$prob * pathways$eff) * discount_factor,
...
for (t_val in t_between_screens) {
  ...
  for (n_scr in num_screens) {
    total_screen_time <- n_scr * t_val
    
    # Filter out impossible combinations that
    # exceed the 20-year time horizon
    if (total_screen_time > time_horizon) next
    
    submodels <- rep(list(dt, mm_scenario), n_scr)
    
    t_remain <- time_horizon - total_screen_time
    if (t_remain > 0) {
      submodels[[length(submodels)]]$n_cycles <- t_val + t_remain
    }
