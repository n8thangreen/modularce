...
run_model.CombinedModel <- function(model, ...) {
  ...
  for (i in seq_along(model)) {
    ...
    results[[i]] <- run_model(curr_model)
  }
  results
}
```
...
The decision tree data are defined as a `tribble()` which allows us to write it in the same format that it will be printed for clearer code (@fig-code-input-dt).
...
markov_model <- function(start_pop,
  ...
  # Cohort trace matrix: rows = cycles (0 to n_cycles), cols = states
  trace <- matrix(NA, nrow = n_cycles + 1, ncol = n_states, 
                  dimnames = list(cycle = 0:n_cycles, state = s_names))
  trace[1, ] <- start_pop
  ...
  # Discount factors
  disc_vec <- 1 / (1 + discount_rate)^(0:n_cycles)
  
  cycle_costs <- (trace %*% cost_vec) * disc_vec
  cycle_eff   <- (trace %*% util_vec) * disc_vec
  
  list(
    final_state   = setNames(as.numeric(trace[n_cycles + 1, ]), s_names),
    trace         = trace,
    expected_cost = sum(cycle_costs),
    expected_eff  = sum(cycle_eff)
  )
}
```
...
```{r}
#| label: fig-ce-plot
#| fig-cap: "Cost-effectiveness plot for a range of screening strategies; screening intervals of 2, 5, 10, 15, 20 years (different colours), and total number of screening episodes 1, 2, 3, 4, 5, 10 (points along curves)."
#| fig-width: 8
#| fig-height: 5
#| echo: false

ce_data <- dplyr::tibble(
  t_interval = factor(vapply(all_results, \(x) x$t_between_screens, numeric(1)), levels = c(2, 5, 10, 15, 20)),
