...
#' @param safe_dir Optional directory that the file must be saved within. Defaults to the current working directory.
#' @export
...
", .open = "<<", .close = ">>")
  ...
  if (!is.null(file)) {
    # Ensure directory exists to safely resolve '..' in all platforms without race conditions
    target_dir <- tryCatch({
    ...
    # ensure trailing slash to prevent bypassing (e.g. allowed: "/foo", target: "/foobar")
