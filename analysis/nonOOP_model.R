...
InputTable       <- data.frame(read.table(file = "inputs/InputTable.txt",       header = T, row.names = 1))  # Apply the first column as row names
DefineStages     <- data.frame(read.table(file = "inputs/Stages.txt",           header = T))  # Stage; Name;Type;Scale;Shape;Utility
Incidence        <- data.frame(read.table(file = "inputs/Incidence.txt",        header = T))  # Age_Interval; Annual_Incidence; Cumulative_Failure
Survival         <- data.frame(read.table(file = "inputs/LifeTable.txt",        header = T))  # Interval_LifeTable; Survival
ScreenSchedule   <- data.frame(read.table(file = "inputs/ScreenSchedule.txt",   header = T))  # ScheduleNumber; StartAge; StopAge; TestApplied1; Interval1
TestPerformance  <- data.frame(read.table(file = "inputs/TestPerformance.txt",  header = T))  # Test; Sn; Sp; Disutility
TreatmentSuccess <- data.frame(read.table(file = "inputs/TreatmentSuccess.txt", header = T))  # PreClinicalProbability; ClinicalProbability
Disutility       <- data.frame(read.table(file = "inputs/OtherDisutility.txt",  header = T))  # PreClinicalProbability; ClinicalProbability
DiscountRates    <- data.frame(read.table(file = "inputs/DiscountRates.txt",    header = T))  # Costs; Effects; DiscountYear
Costs            <- data.frame(read.table(file = "inputs/Costs.txt",            header = T))  # PrimaryScreen; FollowUp; TreatmentScreenDetected; TreatmentClinical
source("inputs/Misc.txt")  # SampleSize; Threshold
source("inputs/Options.txt")
...
for (CurrentRun in c(1: ncol(Input))){
  ...
  write.table(IntermediateOutcomes_AgeDist, 
              paste("outputs/", colnames(Input)[CurrentRun], "_NoScreening", ".txt", sep = ""),
  ...
  for (Strategy in c(1:length(strategies))){
    ...
    # Record intermediate outcomes
    write.table(ScreenCounts, paste("outputs/",
    ...
    write.table(IntermediateOutcomes_AgeDist, paste("outputs/",
                                                    colnames(Input)[CurrentRun], "_", strategies[Strategy], ".txt", sep = ""),
                row.names = F, col.names = T, sep = '\t')
  ...
  write.table(IntermediateOutcomes, paste("outputs/", colnames(Input)[CurrentRun], ".txt", sep = ""), 
              row.names = F, col.names = T, sep = '\t')
  write.table(CostEffectivenessResults, paste("outputs/OutputTable_", colnames(Input)[CurrentRun], ".txt", sep=""), 
              row.names = F, col.names = T, sep = '\t')
  write.table(BoundarySet_L, paste("outputs/LatestResults/BoundarySet_L_", colnames(Input)[CurrentRun], ".txt", sep=""), 
              row.names = F, col.names = T, sep = '\t')
  write.table(BoundarySet_Q, paste("outputs/LatestResults/BoundarySet_Q_", colnames(Input)[CurrentRun], ".txt", sep=""), 
              row.names = F, col.names = T, sep = '\t')
  write.table(BoundarySet_UL, paste("outputs/LatestResults/BoundarySet_UL_", colnames(Input)[CurrentRun], ".txt", sep=""), 
              row.names = F, col.names = T, sep = '\t')
  write.table(BoundarySet_UQ, paste("outputs/LatestResults/BoundarySet_UQ_", colnames(Input)[CurrentRun], ".txt", sep=""), 
              row.names = F, col.names = T, sep = '\t')
  ...
  if ((colnames(Input)[CurrentRun] == "Input") & (CEplane == TRUE)){  # Only plot when it is the base-case
    ...
    jpeg("outputs/Figure.jpeg", width = 4000, height = 2400, res = 300)
    replayPlot(SaveCEplane)
    dev.off()
    
    # Save the graph as a PDF or png if that option is ticked
    if (SaveGraphPDF == TRUE){
      pdf("outputs/Figure.pdf", width = 15, height = 9)
      replayPlot(SaveCEplane)
      dev.off()
    }
    if (SaveGraphPNG == TRUE){
      png("outputs/Figure.png", width = 4000, height = 2400, res = 300)
      replayPlot(SaveCEplane)
      dev.off()
    }
  }
